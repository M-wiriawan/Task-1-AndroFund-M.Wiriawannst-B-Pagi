...
class LoginActivity : AppCompatActivity(), View.OnClickListener {
    ...
    val resultLauncher = registerForActivityResult(ActivityResultContracts.StartIntentSenderForResult()){
        result ->
        if (result.resultCode == Activity.RESULT_OK) {
            val data:Intent? = result.data
            val returString:String? = data?.getStringExtra("history")
            history.text = returString
        }
    }
